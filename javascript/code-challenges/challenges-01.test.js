...
const allUpperCase = (arr) => {
  let capArray = [];
  arr.forEach (caps => {
    let newSentence = caps.toUpperCase();
    capArray.push(newSentence);
  }
  );
  return capArray;
};
...
const addNumbers = (num, arr, times, callback) => { let array = [];
...
const createList = (availableItems) => {
  let list = [];
  availableItems.forEach(actualList => {
    if (actualList.available === true){
      list.push(actualList.name);
